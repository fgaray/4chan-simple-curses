...
instance Show ThreadCatalog where
    show ThreadCatalog{..} = semantic_url ++ " (rep: " ++ show replies ++ ", img: " ++ show images ++ ")"
...
downloadImage url ext = do
    image <- simpleHTTP (getRequest url)
    case image of
        Left err -> return $ show err
        Right rsp -> do
            writeBinaryFile ("4chan" ++ ext)  $! (rspBody rsp)
            readCreateProcess (proc "feh" ["4chan" ++ ext]) ""


findReplies :: [ThreadJSON] -> [(ThreadJSON, [ThreadJSON])]
findReplies []     = []
findReplies (x@ThreadJSON{..}:xs) = (x, findReplies' (show no) xs) : findReplies xs


findReplies' :: String -> [ThreadJSON] -> [ThreadJSON]
findReplies' no = catMaybes . map fn
    where
        fn :: ThreadJSON -> Maybe ThreadJSON
        fn x =
            case com x of
                Nothing -> Nothing
                Just x' -> if (">" ++ no) `isSubsequenceOf` x' then Just x else Nothing
